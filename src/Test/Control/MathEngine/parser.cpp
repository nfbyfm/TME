...
QList <MFFormula *> * Parser::startParsing(QList< QList<MToken *> > tf_list, QList <MFormula *> &f_List, QList <MVariable *> &v_List)
{
        ...
        if(numberOfElements > 0)
            if(tmpPart->getTokenList().at(numberOfElements-1)->getType()==MTokenType::OPERATOR)
                if(tmpPart->getTokenList().at(numberOfElements-1)->getValue()=="/")
                {
                    ...
                    double value=-1;
                    bool toDoubleSuccess = false;
                    value = newElement->getValue().toDouble(&toDoubleSuccess);
                    ...
                    if(toDoubleSuccess)
                    {
                        ...
                        qDebug()<<"Parser: division by Number detected. turned into multiplication.";
                    }
                    else
